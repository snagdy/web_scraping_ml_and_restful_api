...
def write_multiple_html_to_disk_from_list(start, end, generic_filepath, url_list):
    ...
    [write_html_to_disk(url, generic_filepath, pgnum) for url, pgnum in zip(url_sublist, range(start+1,end+1))]
...
if len(pxs) == sum([len(data_series) for data_series in variables])/len(variables):
