...
def get_url_list(generic_url_string, start_page, end_page):
    ...
    return [generic_url_string.format(i) for i in range(start_page, end_page)]
...
def write_multiple_html_to_disk_from_list(start, end, generic_filepath, url_list):
    ...
    [
        write_html_to_disk(url, generic_filepath, pgnum)
        for url, pgnum in zip(url_sublist, range(start + 1, end + 1))
    ]
...
def multi_page_scrape(soup_list, tag, tag_class):
    ...
    return list(
        chain.from_iterable(
            [soup.find_all(tag, class_=tag_class) for soup in soup_list]
        )
    )
...
def parse_date(date_string):
    ...
    return " ".join(
        [
            "".join([char if not char.isalpha() else "" for char in dsl[0]]),
            " ".join(dsl[1:]),
        ]
    )
...
def main():
    url_list = get_url_list(
        "https://nethouseprices.com/house-prices/london?page={}", 1, 12484
    )
    write_multiple_html_to_disk_from_list(0, 40, "page{}_raw.html", url_list)
    soup_kitchen = [read_html_from_disk("page{}_raw.html", i) for i in range(1, 41)]
    ...
    addr = [i.find("a").string.replace("\xa0", " ") for i in addresses]
    pxs = [float(i.string.replace("\xa3", "").replace(",", "")) for i in prices]
    property_characteristics = [
        [i.strip() for i in categories.string.split(",")] for categories in details
    ]
    ...
    top_search_results = [
        result if len(result) == 0 else result[0] for result in json_search_results
    ]
    ...
    geodata_json = [
        convert_json_to_named_tuple(result) for result in top_search_results_as_json
    ]

    # TODO: add data parsed from display_name about borough as alternative to k Mean Clustering, to data frame.
    # json_search_results = [get_geodata_object(url) for url in geodata_urls]
    geodata = [
        load_geodata_attributes(result_named_tuple)
        for result_named_tuple in geodata_json
    ]
    ...
    if len(pxs) == sum([len(dataseries) for dataseries in variables]) / len(variables):
        ...
        dataset = {
            series_name: series for series_name, series in zip(series_names, variables)
        }
